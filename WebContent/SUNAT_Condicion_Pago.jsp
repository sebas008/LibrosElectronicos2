...
<link rel="stylesheet" type="text/css" media="screen"
	href="css/screen.css" />
<!-- ... -->
<title>SUNAT | TABLA CONDICIÓN DE PAGO</title>
</head>
...
<body>
	...
	<a href="<%=request.getContextPath()%>/Nuevo_CondicionPago.jsp"
		TARGET="central"><input type="button" value="Nuevo"
		class="botonNuevo"></a>
	<a
		href="<%=request.getContextPath()%>/Servlet?opcion=listaCondicionPago"
		target="central"><input type="button" value="Refrescar"
		class="botonNuevo"></a>
	<h1>Lista de Condiciones de Pago:</h1>



	<display:table name="${listadoCondicionPago}" pagesize="20"
		export="true" decorator="decorador.Wrapper" id="objeto">
		...
		<display:column title="Modificar">
			<a
				href="<%=request.getContextPath()%>/Servlet?opcion=ObtenerCondicionPago&codigo=<c:out value="${objeto.codigo}"  escapeXml="true"/>">Modificar</a>
		</display:column>
		<display:column title="Eliminar">
			<a
				href="<%=request.getContextPath()%>/Servlet?opcion=EliminarCondicionPago&codigo=<c:out value="${objeto.codigo}"  escapeXml="true"/>">Eliminar</a>
		</display:column>
