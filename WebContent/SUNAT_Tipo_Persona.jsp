...
<link rel="stylesheet" type="text/css" media="screen"
	href="css/screen.css" />
<!-- ... -->
<title>SUNAT | TABLA TIPO PERSONA</title>
</head>
...
<body>
	...
	<a href="<%=request.getContextPath()%>/Nuevo_TipoPersona.jsp"
		TARGET="central"><input type="button" value="Nuevo"
		class="botonNuevo"></a>
	<a href="<%=request.getContextPath()%>/Servlet?opcion=listaTipoPersona"
		target="central"><input type="button" value="Refrescar"
		class="botonNuevo"></a>
	<h1>Lista de Tipo Persona:</h1>



	<display:table name="${listadoTipoPersona}" pagesize="20" export="true"
		...
		<display:column title="Modificar">
			<a
				href="<%=request.getContextPath()%>/Servlet?opcion=ObtenerTipoPersona&codigo=<c:out value="${objeto.codigo}"  escapeXml="true"/>">Modificar</a>
		</display:column>
		<display:column title="Eliminar">
			<a
				href="<%=request.getContextPath()%>/Servlet?opcion=EliminarTipoPersona&codigo=<c:out value="${objeto.codigo}"  escapeXml="true"/>">Eliminar</a>
		</display:column>
