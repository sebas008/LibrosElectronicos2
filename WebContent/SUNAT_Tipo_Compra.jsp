...
<link rel="stylesheet" type="text/css" media="screen"
	href="css/screen.css" />
<!-- ... -->
<title>SUNAT | TABLA TIPO COMPRA</title>
</head>
...
<body>
	...
	<a href="<%=request.getContextPath()%>/Nuevo_TipoCompra.jsp"
		TARGET="central"><input type="button" value="Nuevo"
		class="botonNuevo"></a>
	<a href="<%=request.getContextPath()%>/Servlet?opcion=listaTipoCompra"
		target="central"><input type="button" value="Refrescar"
		class="botonNuevo"></a>
	<h1>Lista de Tipo Compra:</h1>



	<display:table name="${listadoTipoCompra}" pagesize="20" export="true"
		...
		<display:column title="Modificar">
			<a
				href="<%=request.getContextPath()%>/Servlet?opcion=ObtenerTipoCompra&codigo=<c:out value="${objeto.codigo}"  escapeXml="true"/>">Modificar</a>
		</display:column>
		<display:column title="Eliminar">
			<a
				href="<%=request.getContextPath()%>/Servlet?opcion=EliminarTipoCompra&codigo=<c:out value="${objeto.codigo}"  escapeXml="true"/>">Eliminar</a>
		</display:column>
